...
def create_3d_halo_structure(halosext: 'int32[:,:]', nodeidf: 'int32[:,:]', cellidf: 'int32[:,:]', namef: 'uint32[:]',
                             namen: 'uint32[:]',
                             loctoglobn: 'int32[:]', size: 'int32', nbcells: 'int32', nbfaces: 'int32',
                             nbnodes: 'int32'):
...
def face_info_3d(cellidf, centerc, nodeidc, nodeidf, boundaryfaces, facenameoldf, centerf, normalf, mesuref, vertexn,
                 halonidn,
...
def update_pediodic_info_3d(centerf, cellidf, cellnidc, centerc, vertexn, cellidn, nbnodes, nbcells, periodicinfaces,
                            periodicoutfaces,
                            periodicupperfaces, periodicbottomfaces, periodicinnodes, periodicoutnodes,
                            periodicuppernodes, periodicbottomnodes,
  ...
  for i in periodicinfaces:
    cellidf[i][1] = rightb[
      tuple([np.float(centerf[i][0] + maxcoordx), np.float(centerf[i][1]), np.float(centerf[i][2])])]
  ...
  for i in periodicinnodes:
    for j in range(cellidn[i][-1]):
      leftb.setdefault(tuple([np.float(vertexn[i][0]), np.float(vertexn[i][1]), np.float(vertexn[i][2])]), []).append(
        cellidn[i][j])
  for i in periodicoutnodes:
    for j in range(cellidn[i][-1]):
      rightb.setdefault(tuple([np.float(vertexn[i][0]), np.float(vertexn[i][1]), np.float(vertexn[i][2])]), []).append(
        cellidn[i][j])

  for i in periodicinnodes:
    periodicidn[i].extend(
      rightb[tuple([np.float(vertexn[i][0]) + maxcoordx, np.float(vertexn[i][1]), np.float(vertexn[i][2])])])
  for i in periodicoutnodes:
    periodicidn[i].extend(
      leftb[tuple([np.float(vertexn[i][0]) - maxcoordx, np.float(vertexn[i][1]), np.float(vertexn[i][2])])])
  ...
  for i in periodicupperfaces:
    cellidf[i][1] = rightb[
      tuple([np.float(centerf[i][0]), np.float(centerf[i][1]) - maxcoordy, np.float(centerf[i][2])])]
    periodicfidc[cellidf[i][0]] = cellidf[i][1]  # rightb[np.float(centerf[i][1])]
    for j in range(cellnidc[cellidf[i][1]][-1]):
      periodicnidc[cellidf[i][0]].append(cellnidc[cellidf[i][1]][j])
      longper[cellidf[i][0]] += 1
  ...
  for i in periodicuppernodes:
    periodicidn[i].extend(
      rightb[tuple([np.float(vertexn[i][0]), np.float(vertexn[i][1]) - maxcoordy, np.float(vertexn[i][2])])])

  for i in periodicbottomnodes:
    periodicidn[i].extend(
      leftb[tuple([np.float(vertexn[i][0]), np.float(vertexn[i][1]) + maxcoordy, np.float(vertexn[i][2])])])
  ...
  for i in periodicfrontfaces:
    cellidf[i][1] = rightb[
      tuple([np.float(centerf[i][0]), np.float(centerf[i][1]), np.float(centerf[i][2]) - maxcoordz])]
    periodicfidc[cellidf[i][0]] = cellidf[i][1]  # rightb[np.float(centerf[i][1])]
    for j in range(cellnidc[cellidf[i][1]][-1]):
      periodicnidc[cellidf[i][0]].append(cellnidc[cellidf[i][1]][j])
      longper[cellidf[i][0]] += 1
  ...
  for i in periodicfrontnodes:
    periodicidn[i].extend(
      rightb[tuple([np.float(vertexn[i][0]), np.float(vertexn[i][1]), np.float(vertexn[i][2]) - maxcoordz])])
  for i in periodicbacknodes:
    periodicidn[i].extend(
      leftb[tuple([np.float(vertexn[i][0]), np.float(vertexn[i][1]), np.float(vertexn[i][2]) + maxcoordz])])
...
def create_3doppNodeOfFaces(nodeidc: 'uint32[:,:]', faceidc: 'uint32[:,:]', nodeidf: 'uint32[:,:]', nbelements: 'int32',
                            nbfaces: 'int32'):
...
def face_gradient_info_3d(cellidf: 'int32[:,:]', nodeidf: 'int32[:,:]', centergf: 'float[:,:]', namef: 'uint32[:]',
                          normalf: 'float[:,:]',
...
def create_info_3dfaces(cellid: 'int32[:,:]', nodeid: 'int32[:,:]', namen: 'uint32[:]', vertex: 'float[:,:]',
  ...
  for i in range(nbfaces):
    ...
    if nodeid[i][-1] == 3:
      ...
      centerf[i][:] = 1. / 4 * (
          vertex[nodeid[i][0]][:3] + vertex[nodeid[i][1]][:3] + vertex[nodeid[i][2]][:3] + vertex[nodeid[i][3]][:3])
...
def create_3dfaces(nodeidc: 'uint32[:,:]', nbelements: 'int32', faces: 'int32[:,:]',
  ...
  for i in range(nbelements):
    if nodeidc[i][-1] == 4:
      faces[k][0] = nodeidc[i][0];
      faces[k][1] = nodeidc[i][1];
      faces[k][2] = nodeidc[i][2]
      faces[k + 1][0] = nodeidc[i][2];
      faces[k + 1][1] = nodeidc[i][3];
      faces[k + 1][2] = nodeidc[i][0]
      faces[k + 2][0] = nodeidc[i][0];
      faces[k + 2][1] = nodeidc[i][1];
      faces[k + 2][2] = nodeidc[i][3]
      faces[k + 3][0] = nodeidc[i][3];
      faces[k + 3][1] = nodeidc[i][1];
      faces[k + 3][2] = nodeidc[i][2]
      cellf[i][0] = k;
      cellf[i][1] = k + 1;
      cellf[i][2] = k + 2;
      cellf[i][3] = k + 3
      ...
      faces[k][0] = nodeidc[i][0];
      faces[k][1] = nodeidc[i][1];
      faces[k][2] = nodeidc[i][4];
      faces[k][-1] = 3
      faces[k + 1][0] = nodeidc[i][1];
      faces[k + 1][1] = nodeidc[i][2];
      faces[k + 1][2] = nodeidc[i][4];
      faces[k + 1][-1] = 3
      faces[k + 2][0] = nodeidc[i][2];
      faces[k + 2][1] = nodeidc[i][3];
      faces[k + 2][2] = nodeidc[i][4];
      faces[k + 1][-1] = 3
      faces[k + 3][0] = nodeidc[i][0];
      faces[k + 3][1] = nodeidc[i][3];
      faces[k + 3][2] = nodeidc[i][4];
      faces[k + 1][-1] = 3
      faces[k + 4][0] = nodeidc[i][0];
      faces[k + 4][1] = nodeidc[i][1];
      faces[k + 4][2] = nodeidc[i][2];
      faces[k + 4][3] = nodeidc[i][3];
      faces[k + 4][-1] = 4
      cellf[i][0] = k;
      cellf[i][1] = k + 1;
      cellf[i][2] = k + 2;
      ...
      faces[k][1] = nodeidc[i][1];
      faces[k][2] = nodeidc[i][2];
      faces[k][3] = nodeidc[i][3];
      faces[k][-1] = 4
      faces[k + 1][0] = nodeidc[i][0];
      faces[k + 1][1] = nodeidc[i][1];
      faces[k + 1][2] = nodeidc[i][4];
      faces[k + 1][3] = nodeidc[i][5];
      faces[k + 1][-1] = 4
      faces[k + 2][0] = nodeidc[i][1];
      faces[k + 2][1] = nodeidc[i][2];
      faces[k + 2][2] = nodeidc[i][5];
      faces[k + 2][3] = nodeidc[i][6];
      faces[k + 2][-1] = 4
      faces[k + 3][0] = nodeidc[i][2];
      faces[k + 3][1] = nodeidc[i][3];
      faces[k + 3][2] = nodeidc[i][6];
      faces[k + 3][3] = nodeidc[i][7];
      faces[k + 3][-1] = 4
      faces[k + 4][0] = nodeidc[i][0];
      faces[k + 4][1] = nodeidc[i][3];
      faces[k + 4][2] = nodeidc[i][4];
      faces[k + 4][3] = nodeidc[i][7];
      faces[k + 4][-1] = 4
      faces[k + 5][0] = nodeidc[i][4];
      faces[k + 5][1] = nodeidc[i][5];
      faces[k + 5][2] = nodeidc[i][6];
      faces[k + 5][3] = nodeidc[i][7];
      faces[k + 5][-1] = 4
      cellf[i][0] = k;
      cellf[i][1] = k + 1;
      cellf[i][2] = k + 2;
...
def variables_3d(centerc: 'float[:,:]', cellid: 'int32[:,:]', haloid: 'int32[:,:]', ghostid: 'int32[:,:]',
                 haloghostid: 'int32[:,:]',
