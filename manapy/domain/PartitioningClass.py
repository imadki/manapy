...
class Partitioning(PartitioningUtils):
  ...
  def _create_multiple_partitions(self, nb_parts: 'int'):
    node_cellid = utils.create_node_cellid(self.cells, self.nb_nodes)
    node_phyid = utils.create_node_phyid(self.phy_faces, self.nb_nodes)
    #part_vert = manapy_c_api.make_n_part_mesh_nodal(self.cells, nb_parts)
    part_vert = manapy_c_api.make_n_part_mesh_dual(self.cells, nb_parts, 2)
