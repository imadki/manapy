// Bindings for face_gradient_info_3d. Compiled four times, once per
...
// face_param1..3 and face_f1..2 are written in place.
void face_gradient_info_3d_py(CIMat face_cellid, CIMat faces,
  ...
  face_gradient_info_3d(
      ...
      make_view<real_t, 1>(face_param3), make_view<real_t, 2>(face_f1),
      make_view<real_t, 2>(face_f2), make_view<const real_t, 2>(cell_shift));
}

} // namespace

void register_face_gradient_info_3d(nb::module_ &m) {
  m.def("face_gradient_info_3d", &face_gradient_info_3d_py,
        ...
        nb::arg("face_param3").noconvert(), nb::arg("face_f1").noconvert(),
        nb::arg("face_f2").noconvert(), nb::arg("cell_shift"),
        "3D counterpart of face_gradient_info_2d. Raises if a face's "
        "face_name/face_to_phyid can't be resolved, or if its diamond area "
        "is degenerate (zero). Writes into face_air_diamond, "
        "face_param1..3 and face_f1..2 in place.");
}
