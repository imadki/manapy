...
void variables_2d(ArrayView<const real_t, 2> cell_center,
  ...
  for (index_t i = 0; i < nbnode; ++i) {
    real_t i_xx = real_t(0), i_yy = real_t(0), i_xy = real_t(0);
    const real_t nx = nodes(i, 0);
    const real_t ny = nodes(i, 1);

    auto accumulate = [&](real_t cx, real_t cy) {
    ...
    // Periodic boundary (old vertex names). Unified branch with the FULL
    // cell_shift vector, so a corner node (periodic in x AND y) gets both
    // its x- and y-partner cells imaged right -- each partner cell holds
    // its own already-signed shift (zero on the components it's not
    // periodic in).
    ...
    const real_t d = i_xx * i_yy - i_xy * i_xy;
    if (d == real_t(0))
      throw std::runtime_error("variables_2d: div 0");
