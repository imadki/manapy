...
#include "common/cell_gradient_2d_common.hpp"
...
__global__ void cell_gradient_2d_kernel(
  ...
  for (index_t i =
    ...
    cell_gradient_2d_element(
        ...
        cell_halonid, cells, node_periodicid, node_oldname, halo_centvol,
        cell_shift, w_x, w_y, w_z, ghost_faceid);
  }
}
...
void launch_cell_gradient_2d(
  ...
  cell_gradient_2d_kernel<<<blocks, threads, 0, stream>>>(
      w_c, w_ghost, w_halo, w_haloghost, cell_center, cell_cellnid,
      ghost_info_flt, ghost_ext_info_flt, cell_ghostnid, cell_haloghostnid,
      cell_halonid, cells, node_periodicid, node_oldname, halo_centvol,
      cell_shift, w_x, w_y, w_z, ghost_faceid, nbelement);
}
