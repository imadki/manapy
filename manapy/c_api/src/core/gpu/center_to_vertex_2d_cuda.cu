...
#include "common/center_to_vertex_2d_common.hpp"
...
__global__ void center_to_vertex_2d_kernel(
  ...
  for (index_t i =
    ...
    center_to_vertex_2d_node(
        ...
        node_R_x, node_R_y, node_lambda_x, node_lambda_y, node_number,
        cell_shift, w_n, ghost_faceid);
  }
}
...
void launch_center_to_vertex_2d(
    ...
    ArrayView<const real_t, 1> w_ghost,
    ...
    ArrayView<const index_t, 2> node_haloghostid,
    ArrayView<const index_t, 2> node_periodicid,
    ArrayView<const index_t, 2> node_halonid,
    ArrayView<const real_t, 2> nodes,
    ArrayView<const index_t, 1> node_oldname,
    ArrayView<const real_t, 1> node_R_x,
    ArrayView<const real_t, 1> node_R_y,
    ArrayView<const real_t, 1> node_lambda_x,
    ArrayView<const real_t, 1> node_lambda_y,
    ArrayView<const index_t, 1> node_number,
    ArrayView<const real_t, 2> cell_shift,
    ArrayView<real_t, 1> w_n,
  ...
  center_to_vertex_2d_kernel<<<blocks, threads, 0, stream>>>(
      ...
      node_lambda_x, node_lambda_y, node_number, cell_shift, w_n, ghost_faceid,
      nbnode);
}
