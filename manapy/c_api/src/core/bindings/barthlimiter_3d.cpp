// Bindings for the 3D Barth-Jespersen limiter kernel. This translation unit is
...
// place.
void barthlimiter_3d_py(CFVec w_c, CFVec w_ghost, CFVec w_halo, CFVec w_x,
...
void barthlimiter_3d_cuda_py(DCFVec w_c, DCFVec w_ghost, DCFVec w_halo,
  ...
  cuda_check(manapy_cuda_post_launch(), "barthlimiter_3d kernel launch");
}

} // namespace

void register_barthlimiter_3d(nb::module_ &m) {
  m.def("barthlimiter_3d", &barthlimiter_3d_py, nb::arg("w_c"),
        ...
        "3D unstructured mesh. Writes the result into psi in place.");

  m.def("barthlimiter_3d_cuda", &barthlimiter_3d_cuda_py, nb::arg("w_c"),
        nb::arg("w_ghost"), nb::arg("w_halo"), nb::arg("w_x"), nb::arg("w_y"),
        nb::arg("w_z"), nb::arg("psi").noconvert(), nb::arg("face_cellid"),
        nb::arg("cell_faceid"), nb::arg("face_name"), nb::arg("face_haloid"),
        nb::arg("cell_center"), nb::arg("face_center"),
        "GPU (CuPy) Barth-Jespersen slope limiter for a reconstructed cell "
        "gradient on a 3D unstructured mesh. Takes device arrays and writes "
