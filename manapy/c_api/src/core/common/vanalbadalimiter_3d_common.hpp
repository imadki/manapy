...
// Named distinctly from common/vanalbadalimiter_2d_common.hpp's helpers so
// both headers stay safe to include in the same translation unit.
MANAPY_COMPUTE_HOST_DEVICE
real_t vanalbada3d_max(real_t a, real_t b) { return a > b ? a : b; }

MANAPY_COMPUTE_HOST_DEVICE
real_t vanalbada3d_min(real_t a, real_t b) { return a < b ? a : b; }

MANAPY_COMPUTE_HOST_DEVICE
real_t vanalbada3d_abs(real_t a) { return a < real_t(0) ? -a : a; }

// van Albada / Venkatakrishnan slope limiter for a single cell i on a 3D
// unstructured mesh: same neighbourhood-min/max structure as
// barthlimiter_3d_cell (common/barthlimiter_3d_common.hpp), but the per-face
// factor is the smooth function phi(y) = (y^2 + 2y)/(y^2 + y + 2) of the
// Barth argument y = (w_max-w_c)/delta instead of min(1, y). phi(y) ~ y for
// small y and -> 1 for large y, so it stops "clipping" smooth extrema
// (Barth's min(1,y) kills the 2nd order there), giving much smoother
// convergence while staying bounded (<= 1). Drop-in replacement for
// barthlimiter_3d_cell (identical signature). Shared verbatim by the CPU loop
// (cpu/vanalbadalimiter_3d_cpu.cpp) and the CUDA kernel
// (gpu/vanalbadalimiter_3d_cuda.cu). Writes psi at index i in place.
//
...
// vanalbadalimiter_2d_cell (common/vanalbadalimiter_2d_common.hpp).
MANAPY_COMPUTE_HOST_DEVICE
void vanalbadalimiter_3d_cell(
  ...
  for (index_t j = 0; j < nf; ++j) {
    ...
    w_max = vanalbada3d_max(w_max, vanalbada3d_max(left, right));
    w_min = vanalbada3d_min(w_min, vanalbada3d_min(left, right));
  }
  ...
  for (index_t j = 0; j < nf; ++j) {
    ...
    real_t psi_ij = val;
    if (vanalbada3d_abs(delta2) >= real_t(1e-10)) {
      real_t y;
      if (delta2 > real_t(0)) {
        y = (w_max - wc_i) / delta2;
      } else {
      ...
      psi_ij = (y * y + real_t(2) * y) / (y * y + y + real_t(2));
    }

    psi_i = vanalbada3d_min(psi_i, psi_ij);
  }
