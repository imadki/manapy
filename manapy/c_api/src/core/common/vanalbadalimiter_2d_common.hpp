...
real_t vanalbada_max(real_t a, real_t b) { return a > b ? a : b; }

MANAPY_COMPUTE_HOST_DEVICE
real_t vanalbada_min(real_t a, real_t b) { return a < b ? a : b; }

MANAPY_COMPUTE_HOST_DEVICE
real_t vanalbada_abs(real_t a) { return a < real_t(0) ? -a : a; }

// van Albada / Venkatakrishnan slope limiter for a single cell i: same
// neighbourhood-min/max structure as Barth-Jespersen
// (common/barthlimiter_2d_common.hpp), but the per-face factor is the smooth
// function phi(y) = (y^2 + 2y)/(y^2 + y + 2) of the Barth argument
// y = (w_max-w_c)/delta instead of min(1, y). phi(y) ~ y for small y and -> 1
// for large y, so it stops "clipping" smooth extrema (Barth's min(1,y) kills
// the 2nd order there), giving much smoother convergence while staying
// bounded (<= 1). Drop-in replacement for barthlimiter_2d_cell (identical
// signature). Shared verbatim by the CPU loop
// (cpu/vanalbadalimiter_2d_cpu.cpp) and the CUDA kernel
// (gpu/vanalbadalimiter_2d_cuda.cu). Writes psi at index i in place.
//
...
void vanalbadalimiter_2d_cell(
  ...
  for (index_t j = 0; j < nf; ++j) {
    ...
    w_max = vanalbada_max(w_max, vanalbada_max(left, right));
    w_min = vanalbada_min(w_min, vanalbada_min(left, right));
  }
  ...
  for (index_t j = 0; j < nf; ++j) {
    ...
    real_t psi_ij = val;
    if (vanalbada_abs(delta2) >= real_t(1e-8)) {
      real_t y;
      if (delta2 > real_t(0)) {
        y = (w_max - wc_i) / delta2;
      } else {
    ...
    psi_i = vanalbada_min(psi_i, psi_ij);
  }
