...
#include "common/haloghost_value_slip_2d_common.hpp"
...
__global__ void haloghost_value_slip_2d_kernel(
  ...
  for (index_t k =
    ...
    haloghost_value_slip_2d_node(i, u_halo, v_halo, u_haloghost, v_haloghost,
                                 node_haloghostid, ghost_ext_info_int,
                                 ghost_ext_info_flt, BCindex);
  }
}
...
void launch_haloghost_value_slip_2d(
  ...
  haloghost_value_slip_2d_kernel<<<blocks, threads, 0, stream>>>(
      u_halo, v_halo, u_haloghost, v_haloghost, node_haloghostid,
      ghost_ext_info_int, ghost_ext_info_flt, BCindex, d_halonodes, n);
}
