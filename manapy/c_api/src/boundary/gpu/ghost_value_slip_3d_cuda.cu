...
// per-face routine.
__global__ void ghost_value_slip_3d_kernel(
    ArrayView<const real_t, 1> u_c, ArrayView<const real_t, 1> v_c,
    ArrayView<const real_t, 1> w_c, ArrayView<real_t, 1> u_ghost,
    ArrayView<real_t, 1> v_ghost, ArrayView<real_t, 1> w_ghost,
...
void launch_ghost_value_slip_3d(ArrayView<const real_t, 1> u_c,
  ...
  ghost_value_slip_3d_kernel<<<blocks, threads, 0, stream>>>(
      u_c, v_c, w_c, u_ghost, v_ghost, w_ghost, face_cellid, bc_faces, normal,
      n);
}
