...
// (translation of _explicitscheme_convective_3d). Identical in structure to the
// 2D version (see explicitscheme_convective_2d_common.hpp) but reconstructs
// with the full three-component gradient/offset and, for periodic faces, adds a
// third wrapped axis (55/66 = z). Unlike 2D, w_z / wz_halo are used here.
// ConvectiveFaceKind comes from convective_common.hpp, scatter_add from
// common/helpers/scatter.hpp; the same template parameter selects the four face
// variants so the exact same arithmetic runs on host and GPU.
...
MANAPY_COMPUTE_HOST_DEVICE void explicitscheme_convective_3d_face(
  ...
  const real_t w_l =
      w_c(cell_left) + recon * psi(cell_left) *
  ...
  if constexpr (Kind == ConvectiveFaceKind::Boundary) {
    ...
    w_r = w_halo(h) + recon * psi_halo(h) * (wx_halo(h) * r_r0 +
                                             wy_halo(h) * r_r1 +
                                             wz_halo(h) * r_r2);
    ...
    if constexpr (Kind == ConvectiveFaceKind::Periodic) {
      ...
      // owner's frame along the wrapped axis (11/22 = x, 33/44 = y, 55/66 = z).
