...
// advecdiff explicit convective residual for 2D linear advection (translation
// of _explicitscheme_convective_2d in to_convert.py). Same four-face-kind
// structure as the advec solver, with two differences:
//   * the numerical flux is a compile-time template parameter (FluxScheme S),
//     resolved once per launch by the host driver's switch(scheme) -- no
//     per-face branch or indirect call (see common/helpers/numerical_flux.hpp);
//   * an order-1 fast path: at order == 1 the (order-1) reconstruction term is
//     zero, so ALL of it is skipped (no gradient/psi/centre loads), matching
//     the Python fast path's div(phi, ne) memory footprint. Bit-identical to
//     the generic path at order 1.
// ConvectiveFaceKind / scatter_add / numerical_flux come from convective_common.hpp
// and common/helpers/. `face_normal` must be at least 3 columns wide.
