...
#include "common/advecdiff/explicitscheme_convective_3d_common.hpp"

namespace {

// One flux scheme, resolved at compile time (see the 2D driver for the shape).
...
void run(ArrayView<real_t, 1> rez_w, ArrayView<const real_t, 1> w_c,
  ...
  for (index_t k = 0; k < n_inner; ++k)
    explicitscheme_convective_3d_face<ConvectiveFaceKind::Inner, Scheme>(
  ...
  for (index_t k = 0; k < n_periodic; ++k)
    explicitscheme_convective_3d_face<ConvectiveFaceKind::Periodic, Scheme>(
        ...
        face_haloid, face_name, cell_shift, order);

  const index_t n_halo = static_cast<index_t>(d_halofaces.size(0));
  for (index_t k = 0; k < n_halo; ++k)
    explicitscheme_convective_3d_face<ConvectiveFaceKind::Halo, Scheme>(
  ...
  for (index_t k = 0; k < n_boundary; ++k)
    explicitscheme_convective_3d_face<ConvectiveFaceKind::Boundary, Scheme>(
...
// CPU entry point. `scheme` selects the flux once (a single switch) into the
// compile-time-specialised run<FluxScheme>. rez_w is written in place.
...
#define MANAPY_ADVECDIFF_RUN(S)                                                 \
  run<S>(rez_w, w_c, w_ghost, w_halo, u_face, v_face, w_face, w_x, w_y, w_z,    \
         wx_halo, wy_halo, wz_halo, psi, psi_halo, cell_center, face_center,   \
