...
#include "common/advec/time_step_common.hpp"
...
// time_step candidate is >= 0 (cfl, volume, lambda and the 1e6 seed are all
// positive), so this is safe here.
...
__global__ void time_step_kernel(ArrayView<const real_t, 1> u,
  ...
  for (index_t i =
    ...
    const real_t cand =
        time_step_cell(i, u, v, w, cfl, face_normal, cell_volume, cell_faceid);
...
real_t launch_time_step(ArrayView<const real_t, 1> u,
  ...
  time_step_kernel<<<blocks, threads, 0, stream>>>(
      u, v, w, cfl, face_normal, cell_volume, cell_faceid, d_dt, nbelement);
