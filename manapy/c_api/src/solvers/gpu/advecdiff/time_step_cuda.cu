...
// time_step candidate is >= 0 (cfl, volume, lambda, the diffusion term and the
// 1e6 seed are all positive), so this is safe here.
...
real_t launch_time_step(ArrayView<const real_t, 1> u,
  ...
  time_step_kernel<<<blocks, threads, 0, stream>>>(
      u, v, w, cfl, face_normal, cell_volume, cell_faceid, Dxx, Dyy, Dzz, d_dt,
      nbelement);
