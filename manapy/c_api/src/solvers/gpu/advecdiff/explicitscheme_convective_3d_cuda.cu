...
// One thread per face, templated on the face kind AND the flux scheme (see the
// 2D kernel). 3D adds w_z/wz_halo and the 55/66 periodic axis via the shared
// per-face routine.
template <ConvectiveFaceKind Kind, FluxScheme Scheme>
__global__ void convective_3d_kernel(
  ...
  for (index_t k =
       ...
       k < nfaces; k += stride) {
    explicitscheme_convective_3d_face<Kind, Scheme>(
...
void launch_face_list(ArrayView<const index_t, 1> faces, cudaStream_t stream,
  ...
  convective_3d_kernel<Kind, Scheme>
...
#define MANAPY_ADVECDIFF_LAUNCH(S)                                             \
  launch_all<S>(d_innerfaces, d_halofaces, d_boundaryfaces,                    \
                ...
                wz_halo, psi, psi_halo, cell_center, face_center, halo_centvol,\
                face_cellid, face_normal, face_haloid, face_name, cell_shift,  \
                order)
