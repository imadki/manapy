// Bindings for the advecdiff 2D explicit convective residual kernel. Compiled
...
// Argument order matches the Python original, with an explicit `scheme` added
// at the end (the Python binds it into the global _compute_flux instead). w_z /
// wz_halo are unused in 2D but kept for signature parity. rez_w is written in
// place.
...
// GPU version: device (CuPy) arrays ingested zero-copy via DLPack; rez_w is
// written in place on the GPU.
void explicitscheme_convective_2d_cuda_py(
  ...
  cuda_check(err, "explicitscheme_convective_2d kernel");
}
...
void register_advecdiff_explicitscheme_convective_2d(nb::module_ &m) {
  m.def("explicitscheme_convective_2d", &explicitscheme_convective_2d_py,
        ...
        "advecdiff explicit convective residual for 2D linear advection. "
        "`scheme` (0=upwind, 1=centered, 2=rusanov, 3=lax_friedrichs) is "
        "resolved once. Writes the per-cell residual into rez_w in place.");
  ...
  m.def("explicitscheme_convective_2d_cuda",
        ...
        nb::arg("d_boundaryfaces"), nb::arg("d_periodicboundaryfaces"),
        nb::arg("cell_shift"), nb::arg("order"), nb::arg("scheme"),
        "GPU (CuPy) advecdiff explicit convective residual for 2D linear "
        "advection. Takes device arrays and writes rez_w in place on the GPU.");
}
