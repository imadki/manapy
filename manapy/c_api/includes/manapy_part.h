 ...
 *  utils.cpp
 *-----------------------------------------------------------------------*/
idx_t binary_search(const PyArray<idx_t, 1> &arr, idx_t item);
void intersect_arr(PyArray<idx_t, 2> *arr, PyArray<idx_t, 1> *indices, idx_t size, std::vector<idx_t> &intersect_arr);
