  ...
  def test_cell_info(self):
    ...
    for p in range(self.nb_partitions):
      ...
      for i in range(len(d_cell_loctoglob)):
        ...
        # Neighbors by face
        cellfid = self.test_tables.cell_cellfid[g_index]
        cellfid = np.sort(cellfid[0:cellfid[-1]])
        cellfid = cellfid[self.test_tables.part_vert[cellfid] == p]
        ...
        # Neighbors by node
        cellnid = self.test_tables.cell_cellnid[g_index]
        cellnid = np.sort(cellnid[0:cellnid[-1]])
        cellnid = cellnid[self.test_tables.part_vert[cellnid] == p]
        ...
        # Halo by face
        if self.nb_partitions != 1:
          halofid = self.test_tables.cell_cellfid[g_index]
          halofid = np.sort(halofid[0:halofid[-1]])
          halofid = halofid[self.test_tables.part_vert[halofid] != p]
        ...
        # Halo by node
        if self.nb_partitions != 1:
          halonid = self.test_tables.cell_cellnid[g_index]
          halonid = np.sort(halonid[0:halonid[-1]])
          halonid = halonid[self.test_tables.part_vert[halonid] != p]
