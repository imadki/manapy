...
class TablesTestHexa3D:
  def __init__(self, d_cell_loctoglob, g_cell_nodeid):
    ...
    self.cell_vertices = np.zeros(shape=(self.nb_cells, 8, 3), dtype=types.np_float_type)
    ...
    self.cell_halonid = np.ones(shape=(self.nb_cells, 26), dtype=types.np_int_type) * -1
    self.cell_halofid = np.ones(shape=(self.nb_cells, 6), dtype=types.np_int_type) * -1
    self.g_cell_cellnid = np.ones(shape=(self.nb_cells, 27), dtype=types.np_int_type) * -1
    self.l_cell_cellnid = np.ones(shape=(self.nb_cells, 27), dtype=types.np_int_type) * -1
    self.g_cell_cellfid = np.ones(shape=(self.nb_cells, 7), dtype=types.np_int_type) * -1
    self.l_cell_cellfid = np.ones(shape=(self.nb_cells, 7), dtype=types.np_int_type) * -1
    self.cell_nf = np.zeros(shape=(self.nb_cells, 6, 3), dtype=types.np_float_type)

    self.faces_measure = np.zeros(shape=(self.nb_cells, 6), dtype=types.np_float_type)
    self.face_center = np.zeros(shape=(self.nb_cells, 6, 3), dtype=types.np_float_type)
    self.face_normal = np.zeros(shape=(self.nb_cells, 6, 3), dtype=types.np_float_type)
    self.faces_vertices = np.zeros(shape=(self.nb_cells, 6, 4, 3), dtype=types.np_float_type)
    self.g_face_name = np.ones(shape=(self.nb_cells, 6), dtype=types.np_int_type) * -1
    self.l_face_name = np.ones(shape=(self.nb_cells, 6), dtype=types.np_int_type) * -1
    self.g_face_cellid = np.ones(shape=(self.nb_cells, 6, 2), dtype=types.np_int_type) * -1
    self.l_face_cellid = np.ones(shape=(self.nb_cells, 6, 2), dtype=types.np_int_type) * -1

    self.g_node_cellid = np.ones(shape=(self.nb_nodes, 9), dtype=types.np_int_type) * -1
    self.l_node_cellid = np.ones(shape=(self.nb_cells, 8, 9), dtype=types.np_int_type) * -1
    self.node_halonid = np.ones(shape=(self.nb_cells, 8, 9), dtype=types.np_int_type) * -1
    self.g_node_name = np.ones(shape=(self.nb_cells, 8), dtype=types.np_int_type) * -1
    self.l_node_name = np.ones(shape=self.nb_nodes, dtype=types.np_int_type) * -1

    self.face_nodeid = np.ones(shape=(self.nb_cells, 6, 4), dtype=types.np_int_type) * -1
    self.ghost_info = np.ones(shape=(self.nb_ghosts, 7), dtype=types.np_float_type) * -1
    self.g_cell_ghostnid = np.ones(shape=(self.nb_cells, 12+1), dtype=types.np_int_type) * -1
    self.l_cell_ghostnid = np.ones(shape=(self.nb_cells, 12+1), dtype=types.np_int_type) * -1
    self.cell_haloghostnid = np.ones(shape=(self.nb_cells, 12+1), dtype=types.np_int_type) * -1
    self.face_ghostid = np.ones(shape=(self.nb_cells, 6), dtype=types.np_int_type) * -1
    self.face_ghostcenter = np.zeros(shape=(self.nb_cells, 6, 4), dtype=types.np_float_type)
    self.g_node_ghostid = np.ones(shape=(self.nb_nodes, 5), dtype=types.np_int_type) * -1
    self.l_node_ghostid = np.ones(shape=(self.nb_cells, 8, 5), dtype=types.np_int_type) * -1
    self.node_haloghostid = np.ones(shape=(self.nb_cells, 8, 5), dtype=types.np_int_type) * -1
  ...
  def _set_face_nodeid(self, g_cell_nodeid):
    for i in range(0, self.nb_cells):
      ...
      self.face_nodeid[i, 0] = np.array([cell_nodes[[0, 1, 2, 3]]], dtype=types.np_int_type)
      self.face_nodeid[i, 1] = np.array([cell_nodes[[4, 7, 5, 6]]], dtype=types.np_int_type)
      self.face_nodeid[i, 2] = np.array([cell_nodes[[0, 4, 7, 3]]], dtype=types.np_int_type)
      self.face_nodeid[i, 3] = np.array([cell_nodes[[1, 5, 6, 2]]], dtype=types.np_int_type)
      self.face_nodeid[i, 4] = np.array([cell_nodes[[0, 4, 5, 1]]], dtype=types.np_int_type)
      self.face_nodeid[i, 5] = np.array([cell_nodes[[3, 7, 6, 2]]], dtype=types.np_int_type)
