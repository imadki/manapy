...
class Checker2D:
  ...
  def _sort_cell_faces(self, c_nodes, c_faces, c_faces_nodes):
    faces_nodes = []
    nb_element = len(c_nodes)
    for i in range(nb_element):
      faces_nodes.append([c_nodes[i], c_nodes[(i + 1) % nb_element]])
    faces_nodes = np.array(faces_nodes, dtype=types.np_int_type)
  ...
  def test_cell_info(self):
    ...
    for p in range(self.nb_partitions):
      ...
      for i in range(len(d_cell_loctoglob)):
        ...
        ghostn_center = self.test_tables.ghost_info[ghostnid][:, 0:3]
  ...
  def test_node_info(self):
    ...
    for p in range(self.nb_partitions):
      ...
      d_node_ghostcenter_info = self.domain_tables.d_node_ghostcenter_info[p]
      d_node_haloghostcenter = self.domain_tables.d_node_haloghostcenter[p]
      d_node_haloghostcenter_info = self.domain_tables.d_node_haloghostcenter_info[p]
      ...
      for i in range(len(d_cell_loctoglob)):
        ...
        c_cell_nodes = d_cells[i][0:d_cells[i][-1]]
        for k in range(cnb_nodes):
          ghostinfo = np.ones(shape=(2, 11), dtype=types.np_float_type) * -1
          ...
          node_cellid = node_ghostinfo[:, 4].astype(types.np_int_type)
          node_faceid = node_ghostinfo[:, 5].astype(types.np_int_type)
          ...
          ghostinfo[0:nb_ghost, 10] = self.test_tables.face_normal[node_cellid, node_faceid][:, 1] # face_normal_y


          ##########################

          c_ghostinfo = np.ones(shape=(2, 11), dtype=types.np_float_type) * -1
          ...
          c_node_cellid = c_node_ghostcenter_info[:, 0]
          ...
          c_ghostinfo[0:c_nb_ghost, 3] = c_node_ghostcenter_info[0:c_nb_ghost, 1] # face_old_name
        ...
        if self.nb_partitions != 1:
          c_cell_nodes = d_cells[i][0:d_cells[i][-1]]
          for k in range(cnb_nodes):
            haloghostinfo = np.ones(shape=(2, 11), dtype=types.np_float_type) * -1
            ...
            node_cellid = node_haloghostinfo[:, 4].astype(types.np_int_type)
            node_faceid = node_haloghostinfo[:, 5].astype(types.np_int_type)
            ...
            c_haloghostinfo = np.ones(shape=(2, 11), dtype=types.np_float_type) * -1
            ...
            c_node_cellid = c_node_haloghostcenter_info[:, 0]
            ...
            c_haloghostinfo[0:c_nb_ghost, 3] = c_node_haloghostcenter_info[0:c_nb_ghost, 1]  # face_old_name
  ...
  def test_face_info(self):
    ...
    for p in range(self.nb_partitions):
      ...
      for i in range(len(d_cell_loctoglob)):
        ...
        faces_ghostcenter = self.test_tables.face_ghostcenter[g_index][:, 0:2]
        c_faces_ghostcenter = d_face_ghostcenter[c_faces][:, 0:2]
  ...
  def test_halo_info(self):
    ...
    for p in range(self.nb_partitions):
      ...
      self.logger.testing("Halo center", np.testing.assert_almost_equal, halosext_center, d_halo_centvol[:, 0:2], decimal=self.decimal_precision)
