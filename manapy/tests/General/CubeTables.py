...
class CubeTables:
  def __init__(self):
    ...
    mesh = Mesh(get_mesh("cube.msh")[1], 3)
    ...
    self.cell_nf = np.zeros((self.nb_cells, 6, 3), dtype=np.float64)
  ...
  def geometry(self):
    ...
    for face_id in range(self.nb_faces):
      ...
      left_cell = self.face_cellid[face_id, 0]
      if left_cell != -1:
        direction = self.cell_center[left_cell] - center
    ...
    for cell_id in range(self.nb_cells):
      face_ids = self.cell_faceid[cell_id, 0:self.cell_faceid[cell_id, -1]]
      for j in range(self.cell_faceid[cell_id, -1]):
        face_id = face_ids[j]
        normal = self.face_normal[face_id].copy()
        if self.face_cellid[face_id, 0] != cell_id:
          normal *= -1.0
        self.cell_nf[cell_id, j] = normal
