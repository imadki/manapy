...
def init(dim, mesh_file_name):
  float_precision = 'double'
  if GPU_Backend.float_precision == 'float32':
...
domain, ne, u, v, w = init(dim=dim, mesh_file_name=mesh_file_name)
S = Solver(ne, vel=(u, v, w), order=order, cft=0.8,
          diffusion=diffusion, advection=advection, Dxx=0.0, Dyy=0.0, Dzz=0.0, dim = dim)


convert_all_tables(ne)
convert_all_tables(u)
convert_all_tables(v)
convert_all_tables(w)
convert_all_tables(domain)
convert_all_tables(domain.nodes)
convert_all_tables(domain.faces)
convert_all_tables(domain.cells)
convert_all_tables(domain.halos)
convert_all_tables(S)



def assign_functions(exec_type):
  ...
  if exec_type == 'cpu':
    ...
    raise RuntimeError("wrong exec_type value in assign_functions")
...
arr_assign = assign_functions('cuda')
...
def test_4(nb_test):
  ...
  def save_test(out_file, nb_blocks, nb_threads, is_free):
    ...
    taken_time = test_time(40, test_1_1)
    # taken_time = 0
    # nb_iter = 20
    # for _ in range(nb_iter):
    #   taken_time += work(domain, S, (ne, u, v, w), 0.01)
    # taken_time /= nb_iter
    if nb_blocks == 32000000:
      nb_blocks = 'free'
  ...
  with open(f'{root_path}/benchmark_result_{nb_test}.csv', 'w') as out_file:
    out_file.write(f"advection 2d {mesh_file_name} nb_cells: {domain.nbcells}\n")
    
    for nb_threads in arr_nb_threads:
      save_test(out_file, 32000000, nb_threads, True)
