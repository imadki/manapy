from numpy import zeros, fabs, sqrt, int32, float, uint32
...
def explicitscheme_convective_2d(rez_w:'float[:]', w_c:'float[:]', w_ghost:'float[:]', w_halo:'float[:]',
                                 ...
                                 periodicboundaryfaces:'uint32[:]', shift:'float[:,:]',  order:'intc'):
...
def time_step(u:'float[:]', v:'float[:]', w:'float[:]',  cfl:'float', normal:'float[:,:]', 
    ...
    for i in range(nbelement):
        ...
        for j in range(faceid[i][-1]):
            ...
            lam_convect = fabs(u[i]*norm[0] + v[i]*norm[1] + w[i]*norm[2])
            lam += lam_convect
