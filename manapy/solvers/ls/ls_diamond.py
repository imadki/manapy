"""Diamond-scheme finite-volume Laplacian kernels.
...
def setup(dim):
    """Compile the diamond kernels for `dim`. Idempotent per dimension."""
    global _agnostic_done
    if not _agnostic_done:
        global _search_element
        _search_element = compile(_search_element)  # nested helper first
        _agnostic_done = True

    if dim in _dims_done:
        return
    if dim == 2:
        global compute_P_gradient_2d_diamond, get_triplet_2d
