...
def load_library(libname):
    ...
    if sys.platform.startswith('win'):  # Windows system
        ...
        lib_fname = 'lib' + libname + '-5.2.1' + '.so'
...
def load_mumps_libraries():
    mumps_libs['dmumps'] = load_library('dmumps_ptscotch').dmumps_c
    mumps_libs['zmumps'] = load_library('zmumps_ptscotch').zmumps_c
...
class MumpsSolver(object):
    ...
    def set_rcd_distributed(self, ir_loc, ic_loc, data_loc):
        ...
        self._data.update(ir_loc=ir_loc, ic_loc=ic_loc, data_loc=data_loc)
#        self.struct.n = n
        self.struct.nz_loc = ir_loc.shape[0]
        if hasattr(self.struct, 'nnz_loc'):
            self.struct.nnz_loc = ir_loc.shape[0]
        self.struct.irn_loc = ir_loc.ctypes.data_as(mumps_pint)
        self.struct.jcn_loc = ic_loc.ctypes.data_as(mumps_pint)
        self.struct.a_loc   = data_loc.ctypes.data_as(mumps_pcomplex)
        
#        self.set_distributed_assembled_rows_cols(irn_loc, jcn_loc)
#        self.set_distributed_assembled_values(a_loc)

#    def set_rcd_distributed(self, irn_loc, jcn_loc):
