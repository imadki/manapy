...
def _initialisation_SW(h: 'float[:]', hu: 'float[:]', hv: 'float[:]', hc: 'float[:]',
                      Z: 'float[:]', cell_center: 'float[:,:]', choix: 'int'):
  ...
  if choix == 1:
    for i in range(nbelements):
      ...
      Z[i] = np.exp(-((xcent - 0.5) ** 2 + (ycent - 0.5) ** 2) / 0.01)
      h[i] = 1. - Z[i]
...
def _initialisation_of_C_in_block(h: 'float[:]', h_c: 'float[:]', center: 'float[:,:]', block: 'float'):
  nbelements = len(h)
  h_c[:] = 0.
  for i in range(nbelements):
    x1, y1 = 738438, 508880
    ...
    # -----------------------block 2-----------------------------------
      ...
      if (ycent >= yf and ycent <= yf1) and xcent >= 727969 and (ycent <= (xcent * a + b)) and (
              ycent <= (xcent * a11 + b11)):
        h_c[i] = 1 * h[i]
    # -----------------------block 3----------------------------------
    elif block == 3:
      ...
      if ((ycent >= yf and ycent <= yf1) and (ycent >= yf2 and ycent <= yf3) and (ycent <= (xcent * a + b))) and (
              ycent <= (xcent * a11 + b11)) \
    ...
    # -----------------------block 4----------------------------------
      ...
      if ycent >= yf and ycent <= yf1 and ycent <= yff and (ycent <= (xcent * a + b)) and (
              ycent <= (xcent * a11 + b11)):
        h_c[i] = 1 * h[i]
        # -----------------------block 5----------------------------------
    elif block == 5:
...
def _initial_Tr(hc: 'float[:]', volume_Ti: 'float[:]'):
  n = len(hc)
  Q0 = 0.
...
def _concentration_overtime(hc: 'float[:]', center: 'float[:,:]', volume_Ti: 'float[:]', block: 'float'):
  n = len(hc)
  Q = 0.
...
def _print_time(d_t: 'float', niter: 'int', time: 'float', rapport: 'float', resid: 'float', rank: 'int'):
  if rank == 0:
    print(" **************************** Computing ****************************")
    print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$ Saving Results $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
    print("Iteration = ", niter, "time = ", np.float32(time), "time step = ", d_t)
    print("Q/Q0 = ", rapport, "temps de residence", resid)



def _initialisation_SW_bis(h: 'float[:]', hu: 'float[:]', hv: 'float[:]', hc: 'float[:]', Z: 'float[:]',
  ...
  if choix == 200:
    for i in range(nbelements):
      ...
      hu[i] = 0 * h[i]
      hv[i] = 0 * h[i]
      hc[i] = 1 + np.exp(-(xcent ** 2 + ycent ** 2) / (2 ** 2))
  ...
  if choix == 0:
    for i in range(nbelements):
      ...
      hv[i] = 0. * h[i]
      hc[i] = 1. * h[
        i]  # 0.*h[i]#10*exp(-((xcent-2400)**2+(ycent-2400)**2)/(5**2))#+6.5*exp(-((xcent-2400)**2+(ycent-2400)**2)/(264**2))
    ...
    for i in range(nbelements):
      ...
      h[i] = 1 - ((c1 ** 2) / (4 * grav * c2)) * np.exp(2 * f)
      hu[i] = (M * np.cos(alpha) + c1 * (ycent - y0) * np.exp(f)) * h[i]
      hv[i] = (M * np.sin(alpha) - c1 * (xcent - x0) * np.exp(f)) * h[i]
    ...
    for i in range(nbelements):
      ...
      h[i] = 1.
      hc[i] = c_1 * np.exp(-1 * ((xcent - x_1) ** 2
                              + (ycent - y_1) ** 2) / sigma1 ** 2) + c_2 * np.exp(-1 * ((xcent - x_2) ** 2
                                                                                     + (
    ...
    for i in range(nbelements):
      ...
      hc[i] = 5 * np.exp(-1. * ((xcent - 6.) ** 2 + (ycent - 0.) ** 2 + (zcent - 0.) ** 2) / (sigma) ** 2) + 1.
      h[i] = 1.
      hu[i] = 2.
      hv[i] = 0.
      Z[i] = 0.
    ...
    for i in range(nbelements):
      ...
      hc[i] = 100 * np.exp(-((xcent - 460397) ** 2 + (ycent - 591263) ** 2) / (2000 ** 2))  # for geblartar
    ...
    for i in range(nbelements):
      ...
      hv[i] = 0.
      # hc[i] = 100*exp(-((xcent-458210)**2+(ycent-603856)**2)/(1000**2))    # for geblartar
      hc[i] = 100 * np.exp(-((xcent - 500443) ** 2 + (ycent - 598855) ** 2) / (2000 ** 2))  # for geblartar
    ...
    for i in range(nbelements):
      ...
      dR = np.sqrt((xcent - x0) ** 2 + (ycent - y0) ** 2)
      # if (i == 10589):
      # if dR < 1000:
      #    print(dR, i)
