...
def _update_rhs_glob(ne: 'float[:]', ni: 'float[:]', loctoglob: 'int32[:]', rhs_updated: 'float[:]'):
  for i in range(len(ne)):
    rhs_updated[loctoglob[i]] = 1.8096e-8 * (ne[i] - ni[i])


def _update_rhs_loc(ne: 'float[:]', ni: 'float[:]', loctoglob: 'int32[:]', rhs_updated: 'float[:]'):
  for i in range(len(ne)):
    rhs_updated[i] = 1.8096e-8 * (ne[i] - ni[i])


def _explicitscheme_dissipative_ST(u_face: 'float[:]', v_face: 'float[:]', w_face: 'float[:]', Ex_face: 'float[:]',
                                   Ey_face: 'float[:]', Ez_face: 'float[:]', nex_face: 'float[:]', ney_face: 'float[:]',
                                   nez_face: 'float[:]', face_cellid: 'int32[:,:]', face_normal: 'float[:,:]',
                                   face_name: 'int32[:]', dissip_ne: 'float[:]'):
  ...
  for i in range(nbface):
    ...
    if face_name[i] == 0:
      dissip_ne[face_cellid[i][0]] += flux_ne
      dissip_ne[face_cellid[i][1]] -= flux_ne
    else:
      dissip_ne[face_cellid[i][0]] += flux_ne


def _explicitscheme_source_ST(ne: 'float[:]', u: 'float[:]', v: 'float[:]', w: 'float[:]', Ex: 'float[:]', Ey: 'float[:]',
  ...
  for i in range(nbelements):
    ...
    if br == 1:
      S += 1e25 * np.exp(-1. * ((xcent - 0.3) ** 2. + (ycent - 0.25) ** 2. + (zcent - 0.28) ** 2.) / (0.005 ** 2.))

    if br == 2:
      S += 1e25 * np.exp(-1. * ((xcent - 0.31) ** 2. + (ycent - 0.25) ** 2. + (zcent - 0.22) ** 2.) / (0.005 ** 2.))

    src_ne[i] = S
    src_ni[i] = S


def _compute_el_field(Pgradfacex: 'float[:]', Pgradfacey: 'float[:]', Pgradfacez: 'float[:]',
...
def _compute_velocity(Ex_face: 'float[:]', Ey_face: 'float[:]', Ez_face: 'float[:]', u_face: 'float[:]', v_face: 'float[:]',
                      w_face: 'float[:]', Ex: 'float[:]', Ey: 'float[:]', Ez: 'float[:]', u: 'float[:]', v: 'float[:]',
                      w: 'float[:]', cell_faceid: 'int32[:,:]', dim: 'int'):
  n = 2.5e19
  nbfaces = len(u_face)
  nbelements = len(cell_faceid)
  u[:] = 0.
  v[:] = 0.
  w[:] = 0.
  Ex[:] = 0.
...
def _time_step_ST(u: 'float[:]', v: 'float[:]', w: 'float[:]', Ex: 'float[:]', Ey: 'float[:]', Ez: 'float[:]',
                  cfl: 'float', face_normal: 'float[:,:]', face_measure: 'float[:]', cell_volume: 'float[:]',
                  cell_faceid: 'int32[:,:]', dim: 'int'):
  ...
  for i in range(nbelement):
    ...
    for j in range(dim + 1):
      ...
      lam_convect = np.fabs(u[i] * norm[0] + v[i] * norm[1] + w[i] * norm[2]) / face_measure[face]
...
def _update_ST(ne_c: 'float[:]', ni_c: 'float[:]', rez_ne: 'float[:]', rez_ni: 'float[:]',
