...
class TablesTestTetra3D:
  ...
  def _set_l_face_cellid(self, l_face_name, g_face_cellid):
    ...
    for i in range(self.nb_cells):
      ...
      for j in range(nb_faces):
        ...
        if arr[j, 0] != i and name == 10:
          arr[j, 0] = -1
        elif arr[j, 1] != i and name == 10:
          arr[j, 1] = -1
  ...
  def _set_ghost_info(self, face_nodeid, cell_which_partition, cell_center, g_face_name, face_center, face_normal):
    # Set face_ghostid
    # Set g_node_ghostid
  ...
  def _set_node_oldname(self, cell_vertices):
    for i in range(self.nb_cells):
      cellv = cell_vertices[i]
      Width = self.WIDTH
      Height = self.HEIGHT
      ...
      name[cellv[:, 2] <= 0.01] = 6
      name[cellv[:, 1] <= 0.01] = 4
      name[cellv[:, 1] >= Height-0.01] = 3
      name[cellv[:, 0] <= 0.01] = 1
      name[cellv[:, 0] >= Width-0.01] = 2
  ...
  def _set_node_name(self, g_node_name, l_face_name, g_cell_nodeid):
    for i in range(self.nb_cells):
      ...
      for face_id in range(face_nodes.shape[0]):
        face_name = l_face_name[i][face_id]
        for node_id in range(face_nodes.shape[1]):
