...
static int  create_sub_domains(PyArrayObject *graph,
    ...
    for (idx_t i = 0; i < cells_dim[0]; i++) {
        ...
        // Create local cells
        local_domains[p].map_cells[i] = local_domains[p].map_cells[i];
    ...
    for (idx_t p = 0; p < nb_parts; p++) {
        ...
        for (auto iter = map_cells.begin(); iter != map_cells.end(); ++iter) {
            ...
            //copy cell nodes
            const idx_t size = *(idx_t *)PyArray_GETPTR2(cells, k, max_cell_nodeid - 1);
        ...
        for (auto iter = map_nodes.begin(); iter != map_nodes.end(); ++iter) {
            ...
            for (idx_t i = 0; i < 3; i++) {
                l_nodes_data[k * 3 + i] = *(fdx_t *)PyArray_GETPTR2(nodes, k, i);
            }
...
static PyObject *py_create_sub_domains(PyObject *self, PyObject *args) {
    ...
    PySys_WriteStdout("Start Creating SubDomains \n");
    LocalDomainStruct *local_domains = (LocalDomainStruct *)malloc(sizeof(LocalDomainStruct) * nb_part);
    if (!local_domains ||
