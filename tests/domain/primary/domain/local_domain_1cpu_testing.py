import os
import h5py
from create_domain import LocalDomain
...
class LocalDomain1Cpu(LocalDomain):
  ...
  @staticmethod
  def create_local_domains(local_domain_structs: 'LocalDomainStructData[:]'):
    ...
    for rank in range(size):
      ...
      self.cell_nf = self._create_normal_face_of_cell_2d(self.cell_center, self.face_center, self.cell_faceid,
                                                         self.face_normal)
      log_step()
